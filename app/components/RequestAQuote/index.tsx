...
interface IRequestAQuote {
  contact: {
    ...
    form_fields: {
      ...
      enterprise: string;
      country: string;
      select_your_country: string;
      email: string;
      phone: string;
      interest: string;
      select_your_interest: string;
      message: string;
    };
    interests: {
      label: string;
...
export const RequestAQuote = forwardRef<HTMLElement, IRequestAQuote>(
  ({ contact }, ref) => {
    ...
    return (
      ...
      >
        <HeaderSection
          title={contact.title ?? 'Request a quote'}
          subtitle={
            contact.subtitle ??
            'Contact our team to discuss exports, partnerships, and product availability'
          }
            ...
            <div className="flex flex-col md:flex-row items-stretch gap-5 md:gap-10 w-full">
              <div className="w-full lg:flex-1 flex flex-col gap-5">
                <Input
                  id="nome"
                  name="nome"
                  label={contact.form_fields.name ?? 'Name'}
                  value={formData.nome}
                ...
                <Input
                  name="empresa"
                  id="empresa"
                  label={contact.form_fields.enterprise ?? 'Enterprise'}
                ...
                <Select
                  name="pais"
                  options={countries}
                  label={contact.form_fields.country ?? 'Country'}
                  defaultOption={
                    contact.form_fields.select_your_country ??
                    'Select your country'
                  }
                  value={formData.pais}
                  onChange={handleChange}
                ...
                <Input
                  id="email"
                  name="email"
                  label={contact.form_fields.email ?? 'E-mail'}
                  type="email"
                ...
                <Input
                  id="telefone"
                  name="telefone"
                  label={contact.form_fields.phone ?? 'Phone'}
              ...
              <div className="w-full lg:flex-1 flex flex-col gap-5">
                <Select
                  ...
                  label="Interest"
                  defaultOption={
                    contact.form_fields.select_your_interest ??
            ...
            >
              {isLoading ? (
                <>
                  ...
                  {contact.button_text.sending ?? 'Sending'}
                </>
              ) : (
                <>
                  <Send size={18} />
                  {contact.button_text.send ?? 'Send'}
                </>
              )}
            </button>
          </form>
